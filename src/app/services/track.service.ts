...
isFavorite = false;
  ...
  private baseUrl:string = "https://192.168.23.122:8082/api/Track/";
  private favBaseUrl:string = "https://192.168.23.122:8082/api/Favourite/";
...
//Method fetces Favorite bool from API. Bool is true if the song is favorited by the user.
  async function fetchApiAndSetVariable(userId:number,trackId:number): Promise<boolean> {
    try {
      const response = await fetch("https://192.168.23.122:8082/api/Favourite/"+`IsSongFav/${userId}/${trackId}`);
